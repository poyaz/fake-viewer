...
socksport() { local ports="$1" file=/etc/tor/torrc
    local name="SocksPort"
    IFS=',' read -ra ADDR <<< "$ports"
    for port in "${ADDR[@]}"; do
        if grep -q "^$name.*$port.*" /etc/tor/torrc; then
            continue
        else
            echo "$name $port" >>/etc/tor/torrc
        fi
    done
}
...
[[ "${SOCKSPORT:-"9050"}" ]] && socksport "$SOCKSPORT"
